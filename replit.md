...
- Reminder Scheduler System: Automated reminder delivery via external cron job (POST /api/scheduler/reminders). Checks active reminders with scheduled times against user timezones, sends email notifications to primary family members via Resend, and tracks history in `reminder_history` table. Protected by SCHEDULER_SECRET header. Services in `artifacts/api-server/src/services/`: reminder-utils.ts, reminder-email-service.ts, reminder-scheduler.ts.
