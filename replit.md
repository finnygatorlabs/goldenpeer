...
- Daily Quotes: The home screen displays an AI-generated inspirational quote (left-justified, italicized, max 2 lines) above the voice orb before a conversation starts. When the user taps the orb or "Type instead", the quote animates off-screen (slides right with fade). Quotes are generated daily by AI (via Replit AI Integrations / OpenAI gpt-5-nano) and cached in the `daily_quote_cache` DB table. The API endpoint is `GET /api/daily-quote`. A static fallback collection in `constants/dailyQuotes.ts` (60 quotes) is used if the AI call fails. Users can disable this feature via a toggle in Settings. The preference is stored as `daily_quotes_enabled` in the users table.
